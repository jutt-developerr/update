...
if (!function_exists('distance_matrix_with_toll')) {

    function distance_matrix_with_toll($pickup_lat, $pickup_lng, $drop_lat, $drop_lng)
    {

        // Request payload as an array
$request_data = [
    'origin' => [
        'location' => [
            'latLng' => [
...
curl_setopt($curl, CURLOPT_HTTPHEADER, [
    ...
    'X-Goog-Api-Key:'.get_settings('google_map_key_for_distance_matrix'),  // Replace with your API key
...
if(isset($result->error)){
    ...
    return json_encode(['success'=>false,'message'=>$result->error->message]);
}


if (isset($result->routes[0]->legs[0]->travelAdvisory)) {

            $distance_in_unit = $result->routes[0]->distanceMeters / 1000;


            $explodable_duration = explode("s", $duration);

            $duration = round($explodable_duration[0]/60);

            return json_encode(['success'=>true,'toll_price'=>(double)$result->routes[0]->legs[0]->travelAdvisory->tollInfo->estimatedPrice[0]->units,'distance_in_km_meters'=>$distance_in_unit,'duration_in_mins'=>$result->routes[0]->duration]);

        }else{

            $explodable_duration = explode("s", $result->routes[0]->duration);
            ...
            return json_encode(['success'=>true,'toll_price'=>0,'distance_in_km_meters'=>$distance_in_unit,'duration_in_mins'=>$duration]);
        }
