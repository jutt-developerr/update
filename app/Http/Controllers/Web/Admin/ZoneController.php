...
class ZoneController extends BaseController
{
    ...
    public function store(CreateZoneRequest $request)
    {
        ...
        foreach (json_decode($request->coordinates) as $key => $coordinates) {
            ...
            foreach ($coordinates as $key => $coordinate) {
                ...
                if ($check_if_exists) {
                    throw ValidationException::withMessages(['zone_name' => __('Coordinates already exists with our old zone')]);
                }
    ...
    */
    ...
    public function surgeView(Zone $zone)
    {
        ...
        return view('admin.zone.surgeprice', compact('page', 'main_menu', 'sub_menu', 'zone','days'));
    }
        ...
        try {
            if ($request->has('start_time')) {
                ...
                foreach ($request->start_time as $key => $surge) {
                    ...
                    $surge_price = $zone->zoneSurge()->create($data);

                    // $week_day = $request->days[$key];


                    // $surge_price->zoneSurgeDays()->delete();
    ...
    */
     ...
     public function packageUpdate(ZoneTypePackagePriceRequest $request,ZoneTypePackagePrice $package)
    {
         if (env('APP_FOR')=='demo') {
            ...
            return redirect('zone/types/zone_package_price/index/'.$package->zone_type_id)->with('warning', $message);
        }
        // dd($request->all());
